 ...
 * @author SweeTs
 */
public class L2PasswordInstance extends L2NpcInstance
{
	...
	private static boolean conditions(String newPass, String repeatNewPass, L2PcInstance player)
	{
		if (newPass.length() < 3)
		{
	...
	private static void changePassword(String newPass, String repeatNewPass, L2PcInstance activeChar)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement ps = con.prepareStatement("UPDATE accounts SET password=? WHERE login=?"))
		{
			...
			ThreadPoolManager.getInstance().scheduleGeneral(() -> activeChar.logout(false), 3000);
		}
		catch (Exception e)
		{
