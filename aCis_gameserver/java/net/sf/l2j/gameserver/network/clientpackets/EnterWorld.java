...
public class EnterWorld extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (player.isNewChar() && Config.WELCOME_EFFECT)
		{
		...
		if (Config.ANNOUNCE_HEROES)
			if (player.isHero())
				Broadcast.announceToOnlinePlayers("Hero " + player.getName() + " has been logged in.", true);
		...
		// Load quests.
		try (Connection con = L2DatabaseFactory.getInstance().getConnection(); PreparedStatement ps = con.prepareStatement(LOAD_PLAYER_QUESTS))
		{
		...
		{
			player.sendPacket(new CreatureSay(0, Say2.TELL, Config.PM_TEXT1, Config.PM_SERVER_NAME));
			player.sendPacket(new CreatureSay(0, Say2.TELL, player.getName(), Config.PM_TEXT2));
		}
		...
		else if (Config.SERVER_NEWS && player.getPincheck())
		{
			final NpcHtmlMessage html = new NpcHtmlMessage(0);
